...
steps<-steps[-1]
len=length(steps)
cl=data.frame(indicator=rep("cl",len),values=rep(0,len),p=steps)
apl=data.frame(indicator=rep("apl",len),values=rep(0,len),p=steps)
di=data.frame(indicator=rep("di",len),values=rep(0,len),p=steps)
ntrials=100
for (i in 1:len){
...
to_plot<-rbind(cl[10:len,],apl[10:len,],di[10:len,])
ggplot(to_plot)+aes(x=p,y=values,colour=indicator)+geom_line()+geom_vline(xintercept = 0.689)+
  ...
  geom_hline(yintercept = 1.31107,colour="red",linetype="dashed")+
  geom_hline(yintercept = 0.7758672,colour="green",linetype="dashed")
