...
tmp <- lapply(seq_along(groups), function(u){plot(groups[[u]],main = paste(names(groups)[u]),vertex.label=NA)})
```
...
## Summary table of descriptive measures

```{r}
g.table <- data.frame(Global=c(average.path.length(g), diameter(g), transitivity(g)))
row.names(g.table) <- c("APL", "Diameter", "Clustering")
