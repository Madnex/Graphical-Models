...
- id: floodinglink
  title: Flooding
  URL: 'https://www.cs.yale.edu/homes/aspnes/pinewiki/Flooding.html'
  issued: 
    -year: 2020
---
...
groups <- list(Cleaners=cleaners, Foragers=foragers, Nurses=nurses)

par(mfrow=c(1,3))
colors_ <- categorical_pal(5)[2:4]
tmp <- lapply(seq_along(groups), function(u){
  degs <- rescale(degree(groups[[u]]), c(0,1))
  plot(groups[[u]], main = paste(names(groups)[u]), vertex.label=NA, vertex.size = degs * 13, vertex.color=colors_[u])
  })
```
...
atributes1<-data.frame(
  ...
  category=rep("age",length(atributes$name)),
  val=atributes$`age(days)`
)
atributes2<-data.frame(
  id=atributes$name,
  group=atributes$group,
  category=rep("size",length(atributes$name)),
...
body_size_boxplot <- ggplot(atr[atr$category=="size",])+aes(x=group,y=val)+geom_boxplot()
body_size_boxplot <- body_size_boxplot + labs(x="Group", y="Size (mm)",title="Boxplot of Body Size")

body_size_hist <- ggplot(atr[atr$category=="size",])+aes(x=val)+geom_histogram(binwidth=10)+xlab("size")
...
atr_size<-atr[atr$category=="size",]
...
```{r echo=FALSE, fig.width=7, fig.height=2}
age_boxplot <- ggplot(atr[atr$category=="age",])+aes(x=group,y=val)+geom_boxplot()
age_boxplot <- age_boxplot + labs(x="Group", y="Age (days)",title="Boxplot of Age")

age_hist <- ggplot(atr[atr$category=="age",])+aes(x=val)+geom_histogram(binwidth=10)
age_hist <- age_hist + labs(x="Age (days)", y="Count",title="Histogram of Age")
...
The boxplots show that most ants have similar ages, with the exception of the queen which is much older than the other ants. The groups do not seem to be that different in age. However, we can note a small tendency of nurses being a bit younger ants, yet all groups overlap a lot.
...
```{r echo=FALSE }
knitr::kable(list("Shapiro Tests Age"=results_age, "Shapiro Tests Size"=results_size),
             caption = "Shapiro Test Results (normality thesis)")
```
...
plot_deg_cent <- plot_deg_cent + labs(title = "Degree Centr.", y="Value")
...
plot_closeness_cent <- ggplot(deg)+aes(y=closeness_centrality,x=index)+geom_col(width=1)+
  theme(axis.title.x = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
plot_closeness_cent <- plot_closeness_cent + labs(title = "Closeness Centr.", y="Value")
...
plot_betweenness_cent <- ggplot(deg)+aes(y=betweenes_centrality,x=index)+geom_col(width=1)+
  theme(axis.title.x = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
plot_betweenness_cent <- plot_betweenness_cent + labs(title="Betwn. Centr.", y="Value")
...
plot_ev_cent <- ggplot(deg)+aes(y=eigenvalue_centrality,x=index)+geom_col(width=1)+
  theme(axis.title.x = element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
plot_ev_cent <- plot_ev_cent + labs(title="Eigenval. Centr.", y="Value")
...
```{r, echo=FALSE, fig.width=7,fig.height=2, include=FALSE}
tmp <- lapply(seq_along(groups), function(u){
...
boxplot(lapply(groups, degree), ylab="Degree", main="Degree by Group")
```
...
sum_withinn<-do.call(rbind,lapply(1:length(models),
                                  function(i){models[[i]]$tot.withinss}))
sum_withinn<-data.frame(index=1:nrow(sum_withinn),
                        name=rep("withiness",nrow(sum_withinn)),value=sum_withinn)
...
However, we considered other network models as well but they depend on restrictions which are not fulfilled in this network. This includes exemplary the normality assumption for correlation networks and gaussian graphical models. 
...
*Jan Lennartz*: 
I performed the statistical analysis by group and took care of the main plot of the network. I conducted the different models and analyzed the ER and ERGM models. Additionally, I introduced our main questions, researched the data and prepared the data (helpers.R).
